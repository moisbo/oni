...
To serve Oni from a subdirectory such as `/oni/`, set the Vite base path in a
root-level `.env` file for local development:

```env
VITE_BASE_PATH=/oni/
```

`VITE_BASE_PATH` is read during the build. Vite then exposes the resulting path
through `import.meta.env.BASE_URL`; `BASE_URL` is generated by Vite and should
not be added to `.env` directly.

For a production build, put the value in `.env.production` or set it in the
build environment, then build the application:

```sh
VITE_BASE_PATH=/oni/ pnpm build
```

The generated `dist` directory must be served at `/oni/`. The web server must
also serve the generated `index.html` for application routes, and make these
files available under the same prefix:
...
The optional `ui.urlPrefix` field in `configuration.json` should match the
deployment prefix without the trailing slash:
...
Normally `VITE_ONI_CONFIG_PATH` is not required. Set it only when the runtime
configuration is hosted at a different path, for example:

```env
VITE_ONI_CONFIG_PATH=/oni/configuration.json
```
